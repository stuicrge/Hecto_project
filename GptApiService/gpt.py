...
def call_gptapi(data):
    response = openai.ChatCompletion.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"{data} 이 후기 내용은 '매우좋음', '좋음', '보통', '나쁨', '매우나쁨' 중에 무엇인가요? 단어로만 답변해주세요. 후기 내용이 없으면 '후기없음'으로 답해주세요."}
        ],
        timeout = 10    
        )
    return response
...
def gpt(csv_len):
    query = f"SELECT productName, title, content FROM ttobak_review ORDER BY id DESC LIMIT {csv_len}"
    ...
    df.to_csv("C:\ReviewService\GptApiService\gpt_answer.csv", encoding="utf-8-sig")


if __name__ == "__main__":
	query = "SELECT count(*) FROM ttobak_review"
	cursor.execute(query)
